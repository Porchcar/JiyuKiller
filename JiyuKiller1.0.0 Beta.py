# -*- encoding:utf-8 -*-
INSIDE_VERSION = "4.0 Beta"
INSIDE_VERSION_PROMPT = "极域终结者4.0 Beta Command Line Version"
VERSION = "1.0.0 Beta"
VERSION_PROMPT = "极域终结者1.0.0 Beta Command Line Version"
...
def suspend_process(program:int|str|list):
    if not program:
        return -1
...
def resume_process(program:int|str|list):
    if not program:
        return -1
    if isinstance(program,int):
        pid = program
...
try:
    ...
    argv_parser_module = '''def parse_arguments(argv:list,valid_identifiers:list,return_type:str):
    """
    解析命令行参数。
...
if not os.path.exists("极域终结者临时文件"):
    os.mkdir("极域终结者临时文件")#创建临时文件夹

if (not os.path.exists("favicon.ico")) or (not os.path.exists("极域终结者临时文件/favicon.ico")):
    file = open("极域终结者临时文件/favicon.ico","wb")
...
logging_file = open("极域终结者临时文件/"+str(d.getFormatTime("%Y-%m-%d"))+".log","a",encoding="utf-8")
sys.stderr = logging_file

if CONFIGS["--help"]:
    print(f"""
    极域终结者{VERSION}.py 帮助文档
    --secret 启动极域终结者时隐藏窗口
...
if CONFIGS["--version"]:
    print(f"极域终结者{VERSION}.py 版本号: {VERSION}\n{VERSION_PROMPT}")
    exit()

if CONFIGS["--createLaunchScript"]:
    file = open("launch.bat","w",encoding="utf-8")
    file.write("@echo off\n")
    a = argv
    a.remove("--createLaunchScript")
    file.write("start %s"%" ".join(a))
    del a
    print("Create launch script success.")
    exit()

if CONFIGS["--createShortcut"]:
    try:
...
def create_desktop_shortcut(target_path, shortcut_name, description="", icon="",working_dir=None):
    ...
    '''
        ...
        try:
            ...
            from createShortcut import create_desktop_shortcut
    create_desktop_shortcut(os.path.abspath(__file__),"极域终结者3.0")
    exit()
...
def check_position():
    ...
    if int(x) == 0 and int(y) == 0 and move_to_close.get() == 1:
        ...
        for i in range(lis.count('')):
            lis.remove('')
        messagebox.showinfo(title="控制台输出",message=os.popen("taskkill /pid %s /f /t"%lis[1]).read())
        flash()
...
def close_taskkill():
    ...
    for i in range(lis.count('')):
        lis.remove('')
    if messagebox.askyesno(title="发现程序！",message="发现可疑程序!\n映像名称：%s\nPID：%s\n会话名：%s\n内存使用：%s\n是否taskkill？"%(lis[0],lis[1],lis[2],lis[3])):
        messagebox.showinfo(title="控制台输出",message=os.popen("taskkill /pid %s /f /t"%lis[1]).read())
    flash()
...
def flash():
    l.config(text="运行中" if scan_starting() else "不在运行",fg="green" if scan_starting() else "red")
...
i = easyStray.Items()
i.add("免费内测版本3.0，作者：张泊桥",lambda:None,enabled=False)
i.add("杀死极域", lambda: os.popen("taskkill /f /im StudentMain.exe"))
if CONFIGS["--autohide"] or CONFIGS["--secret"]:
    i.add("显示窗口", showRoot)
icon = easyStray.StrayIcon(i,"极域终结者3.0","favicon.ico",whenProgramClose)
icon.run(True)
window = IntVar()
mini = IntVar()
nohook = IntVar()
move_to_close = IntVar()
if CONFIGS["--moveToClose"]:
...
if CONFIGS["--secret"]:
    ...
    root.withdraw()
root.title(f"极域终结者{VERSION}")
if os.path.exists("favicon.ico"):
    root.iconbitmap("favicon.ico")
else:
    root.iconbitmap("极域终结者临时文件/favicon.ico")
